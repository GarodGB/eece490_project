...
- **Student**: username, email, password_hash, major, current_semester, strategy, workload_tolerance, optional **target_semester_gpa**, gpa, is_admin, timestamps.
...
- `PUT /api/student/profile`: Updates major, strategy, **workload_tolerance** (0–1), **target_semester_gpa** (optional, 0–4), current semester, etc.
...
- `GET /api/recommendations?credits=...&max_courses=...&term=...`: Recommended next-semester courses. Uses **strategy**, **current GPA**, **target_semester_gpa**, **workload_tolerance**, prerequisites, and difficulty/ML signals. Optional query overrides for one request: **`tolerance`**, **`target_gpa`** (same meaning as profile fields). Response includes **`planning_params`** (e.g. `goal_mode`, `easy_preference_weight`, credit/course caps).
...
- Loads the **student**, **current GPA**, **target_semester_gpa** (optional), and **workload_tolerance** (0–1). Computes a **goal context** (`_compute_goal_planning`): **`goal_mode`** (e.g. `recovery`, `raise_gpa`, `maintain`, `relaxed_target`, `tolerance_only`), **credit slack**, **effective max courses/credits**, **max “Hard” courses** per term, and **`easy_preference_weight`** (how strongly to favor easier/safer picks).
- Gets **unlocked courses** via `get_unlocked_courses(student_id, filter_by_major=True)`; drops completed and invalid credits.
- For each candidate: blends cache + ML difficulty; adjusts for GPA gap vs target, tolerance, and strategy (**easy** / **balanced** / **fast**). Ranking applies an **easy_boost** derived from **`easy_preference_weight`** so “raise GPA” / recovery paths prefer lighter mixes; high GPA + high tolerance + relaxed targets can allow harder courses.
- **Greedy packing** respects per-term credit cap, **`max_hard_courses`**, and difficulty buckets.
- Returns **`planning_params`** in the API payload so the UI can explain the plan (mode, safety weight, caps).

**optimize_semester_plan** takes course IDs and returns predicted semester difficulty and overload risk (semester workload model).
