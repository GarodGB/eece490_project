...
- We maintain **`README.md`** (install/run), **`docs/PROJECT_EXPLANATION.md`** (behavior and flow), **`docs/ML_MODELS.md`** (model-to-code map). What **minimum depth** do you want in the main report vs “appendix” (e.g. full API list, full DB schema)?
...
See also **`PROJECT_GUIDE.md`** (repository root) §3 for discussion-style questions (team prep / exam-style), separate from these supervisor check-ins.
